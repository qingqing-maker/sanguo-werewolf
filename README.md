...
- **三档 AI 强度**：`novice` 新手、`standard` 标准、`expert` 高阶。通过 prompt、记忆窗口、关键事实注入控制难度，不改变桌游规则。
- **实时 Web UI**：WebSocket 推送阶段、发言、内心、夜间行动、投票、死亡和终局事件。
- **TTS 播报**：火山 TTS 或 Edge TTS 按人物音色播报公开发言，失败时回退浏览器语音。
...
- **事件回放**：仅在当前没有活动对局时向重连客户端回放最近一局的公共 UI 日志；不会恢复后端 Agent 状态，也不承诺补齐活动对局断线期间的全部事件。
...
- **AI 强度**：新手、标准、高阶，观战和参战均生效；重开沿用当前档位。

人类输入通过 WebSocket 的 `human_input` 返回；暂停、继续、重开、语音开关和终局回放均由前端控制。

WebSocket 使用连接级随机 256-bit token。服务端仅保存 session token 摘要；同 token 可跨标签页或断线重连到同一进程内 session，并恢复当前 pending input。没有房间时，首个主动发送 `create_room` 的会话创建房间并自动成为主持人，而不是“首个打开网页的人”自动获得权限。单实例只允许一个房间；控制命令和设置接口仅房间创建者可用，匿名 HTTP 状态始终是受限观众投影。TTS 状态/合成接口要求任意有效 session 的 `Authorization: Bearer <session-token>`；`human_input` 还必须匹配服务端座位、gameId 和 requestId。会话仅在当前进程内有效，不提供账号、多房间或跨重启续局；公共回放不含 owner 私密事件、夜间全知事件或内心信息。

### TTS 安全边界

- `/api/tts/status` 与 `/api/tts` 都要求有效 session Bearer token；浏览器使用 WebSocket 认证所用的同一 token。
...
- `src/server/EventLog.ts`：按 gameId 保存最近的公共 UI 事件；校验严格递增 `sequence`，并在无活动局时用于回放。
...
- 活动对局重连依赖 `authenticated` 权威握手快照：恢复当前受限状态、人类座位私密快照和 pending input，并从此刻继续收实时事件；服务器**不会**为活动对局读取或发送 EventLog 历史，所以不宣称补齐断线窗口中的全部事件。旧 `connected` transport 仅兼容显示消息，不再驱动状态。
...
- `start_game` 在已有启动、运行或重开操作时明确返回 `busy`，不会隐式取消旧局。`restart_game` 会先同步占位、取消旧引擎并等待其真实退出，再创建新一代引擎；并发的第二次 start/restart 均返回 `busy`。当前页面“再来一局”按钮采用 cancel 后回欢迎页的产品语义，不会偷偷立即发送 `restart_game`。
...
每份 JSONL 的 `meta` 行会记录**配置指纹**：prompt 版本（`BaseAgent.ts` 内容哈希）、模型 slug、AI 难度、失误注入率、节奏设置、运行环境、随机种子。多文件对比时会列出所有差异字段——只有当差异**正好是**你想验证的那一项时，胜率对比才有意义。指纹中不含 API Key。
...
该结果是当前 `MockProvider`、prompt、规则和随机算法的**可重复工程基线**，不是人类胜率，也不是任何真实 LLM 的能力结论。`standard` 样本中好人胜率低于另外两档，说明当前 Mock 的双阵营技能参数不是严格单调难度刻度；在更多 seed 或真实 Provider 验证前，不据这 100 局过拟合调参。

批量模拟会消耗 Mock 或真实 Provider 预算，执行真实模拟前必须确认 Provider、period、账本和云端余额；本项目不会自动部署到 GitHub，也不执行外部发布。

## 项目边界

- 服务器重启会丢失后端内存中的 Agent、memory、暂停点和决策上下文；事件回放仅恢复观众 UI。
- LLM 输出不是可信程序数据，所有 ID、目标、JSON 和阶段操作都必须经过校验与兜底。
- AI 难度目前是一局一个全局档位，不支持每个座位单独配置。
- `novice/standard/expert` 已有固定 seed Mock 工程基线，但它不是人类或真实模型胜率；难度名称仍需多 seed 与少量受预算保护的真实 Provider 样本校准。
